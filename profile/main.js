 import { initializeApp } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js";
      ...
      import { getFirestore, collection, addDoc, doc, updateDoc, deleteDoc, serverTimestamp, query, orderBy, onSnapshot } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
      ...
      function openAddMenu(client = null) {
          if (client) {
              ...
              document.getElementById('sh-width').value = client.measurements?.shoulderWidth || '';
              document.getElementById('sh-length').value = client.measurements?.shoulderLength || '';
              document.getElementById('back-width').value = client.measurements?.backWidth || '';
              document.getElementById('back-waist-length').value = client.measurements?.backWaistLength || '';
              document.getElementById('neck-girth').value = client.measurements?.neckGirth || '';
              document.getElementById('sleeve-length').value = client.measurements?.sleeveLength || '';
              document.getElementById('arm-girth').value = client.measurements?.armGirth || '';
              ...
              document.getElementById('outseam').value = client.measurements?.outseam || '';
              document.getElementById('thigh').value = client.measurements?.thigh || '';
              document.getElementById('crotch-depth').value = client.measurements?.crotchDepth || '';
              document.getElementById('full-length').value = client.measurements?.fullLength || '';
      ...
      function renderClients(clients, filter = '') {
          ...
          const filteredClients = normalizedFilter
            ? clients.filter((client) => (client.name || '').toLowerCase().includes(normalizedFilter))
            : clients;
          ...
          clientsContainer.innerHTML = filteredClients.map((client) => {
              return `
          ...
          clientsContainer.querySelectorAll('.client-card').forEach((card) => {
              card.addEventListener('click', () => {
                  ...
                  detailMeasurements.innerHTML = measurementOrder.map(({ key, label }) => {
